...
/**
 * Detail component that displays information about a movie or an actor.
 * Uses URL parameters to determine which content to show.
...
function Detail() {
  ...
  if (!pelicula) {
    return (
      <p className="text-center text-red-600 mt-8">
        No se ha encontrado la película solicitada.
      </p>
    );
  }
  ...
  if (idInterprete !== undefined) {
    ...
    if (!actor) {
      return (
        <p className="text-center text-red-600 mt-8">
          No se ha encontrado el intérprete solicitado.
        </p>
      );
    }
